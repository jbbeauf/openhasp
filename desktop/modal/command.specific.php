...
/* Charger le fichier contenant la définition de toutes les commandes spécifiques disponibles */
$file = __DIR__ . '/../../data/Objects.json';
if (file_exists($file)) {
    $json = file_get_contents($file);
    if ($json === false) {
        throw new Exception(__('Fonction non disponible', __FILE__) . ' - ' . __('Erreur en chargeant le fichier de définition des commandes spécifiques', __FILE__) . ' : ' . __('Ré-installer le plugin', __FILE__));
    }
    ...
    if ($json_data === null) {
        throw new Exception(__('Fonction non disponible', __FILE__) . ' - ' . __('Erreur en décodant le fichier de définition des commandes spécifiques', __FILE__) . ' : ' . __('Ré-installer le plugin', __FILE__));
    }
} else {
    throw new Exception(__('Fonction non disponible', __FILE__) . ' - ' . __('Fichier de définition des commandes spécifiques non trouvé', __FILE__) . ' : ' . __('Ré-installer le plugin', __FILE__));
}

$objects = array();
$objectsFromJsonl = $eqLogic->importCommands();
$cheatCodeGetDynamicProperty = '#GET#';
foreach ($objectsFromJsonl as $objectJsonl) {
    /* Pour chaque objet jsonl récupéré dans l'écran */
    if (array_key_exists($objectJsonl["obj"], $json_data)) {
        /* Si l'objet est supporté */
        $jsonlObjectId = 'p' . $objectJsonl['page'] . 'b' .  $objectJsonl['id'];

        /* Informations générales sur l'ojbet */
        $objects[$jsonlObjectId] = array();
        $objects[$jsonlObjectId]['name'] =  $json_data[$objectJsonl['obj']]['name'];
        $objects[$jsonlObjectId]['displayName'] =  '{{Page}}' . ' ' . $objectJsonl['page'] . ' ' . $json_data[$objectJsonl['obj']]['name'] . ' ' . $jsonlObjectId;
        if (array_key_exists('text', $objectJsonl)) {
            $objects[$jsonlObjectId]['displayName'] .= ' ({{Affiché}} = "' . $objectJsonl['text'] . '")';
            
        } 
        $objects[$jsonlObjectId]['page'] = $objectJsonl['page'];

        /* Ajout de toutes les commandes possible de l'objet : spécificques à l'objet et générales puis tous les objets */
        $objects[$jsonlObjectId]['command'] = array();
        foreach ($json_data[$objectJsonl['obj']]['command'] as $command) {
            /* Permet de récupérer en dynamique des informations de l'objet définit dans l'écran */
            foreach ($command as $keyCommand => $valueCommand) {
                if (substr($valueCommand, 0, strlen($cheatCodeGetDynamicProperty)) === $cheatCodeGetDynamicProperty) {
                    $propertiesFromObject = explode('#', str_replace($cheatCodeGetDynamicProperty, '', $valueCommand));
                    switch (count($propertiesFromObject)) {
                        case 1:
                            if (array_key_exists($propertiesFromObject[0], $objectJsonl)) {
                                $command[$keyCommand] = $objectJsonl[$propertiesFromObject[0]];
                            }
                            break;
                        case 2:
                            if (array_key_exists($propertiesFromObject[0], $objectJsonl)) {
                                ...
                                $command[$keyCommand] = $propertiesFromObject[1];
                            }
                            break;
                        default:
                            break;
            ...
            $command['topic'] = str_replace('pXbY', $jsonlObjectId, $command['topic']);
            $command['cmdLinked'] = '';
            $command['group'] = 'cmdSpecific';
            array_push($objects[$jsonlObjectId]['command'], $command);
        }
        foreach ($json_data['all']['command'] as $command) {
            $command['topic'] = str_replace('pXbY', $jsonlObjectId, $command['topic']);
            $command['cmdLinked'] = '';
            $command['group'] = 'cmdGeneral';
            array_push($objects[$jsonlObjectId]['command'], $command);
        }
    }
}

/* Liste de toutes les commandes exitantes : commandes de type 'spécifique'*/
$listAvailableCommands = array();
foreach ($eqLogic->getCmd() as $cmd) {
    if ( 'specific' == $cmd->getConfiguration('type')) {
        $listAvailableCommands[$cmd->getId()]['name'] = $cmd->getName() . ' (' . $cmd->getId() . ')';
        $listAvailableCommands[$cmd->getId()]['id'] = $cmd->getId();
    }
}

/* Lien entre les commandes spécifiques disponibles et les commandes existantes */
foreach($objects as $groupKey => $groupElement) {
        ...
        foreach ($eqLogic->getCmd() as $cmd) {
            if ( 'specific' == $cmd->getConfiguration('type')) {
                if ($commandElement['topic'] == $cmd->getLogicalId()) {
                    $objects[$groupKey]['command'][$commandKey]['checked'] = 'checked';
                    $objects[$groupKey]['command'][$commandKey]['cmdLinked'] =  $cmd->getId();;
                    break;
                } else {
                    $objects[$groupKey]['command'][$commandKey]['checked'] = 'unchecked';
                    $objects[$groupKey]['command'][$commandKey]['cmdLinked'] = '';
                }
            }
        }
    }
...
sendVarToJS('tableElements', $objects);
?>
...
<div class="modal-content">
    ...
    <!-- <div class="overflow-auto vertical-scrollable;"> -->
            <table id="table_manage_cmd_specific" class="table table-bordered">
                <thead>
                ...
                <?php
                    foreach($objects as $groupKey => $groupElement) {
                        ...
                        echo '<td colspan="3">';
                        echo '<div class="cursor card logoPrimary" data-toggle="collapse" data-target="#group_' . $groupKey . '" aria-expanded="false">';
                        echo '<h3 class="display-3"><i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i>&nbsp;&nbsp;' . $groupElement['displayName'] . '</h3>';
                        echo '</div>';
                        echo '</td>';
                        echo '</tr>';
                        echo '</tbody>';
                        ...
                        foreach($groupElement['command'] as $commandKey => $commandElement) {
                            ...
                            echo '<td>';
                            echo '<div class="' . $commandElement['group'] . '">';
                            echo '<div style="display: none;">' . $groupElement['displayName'] . ' </div>'; // Pour la recherche
...
<script>
    ...
    $('.selectCommand, .chkSelectCommand').change(function (e) {
        var arrayAllValues = [];
        $('#table_manage_cmd_specific').find("select").each(function() {
    ...
    $( document ).ready(function() {
        ...
        $("#filterElements").on("keyup", function() {
            ...
            $("#table_manage_cmd_specific tr:not(.sectionTitle)").filter(function() {
    ...
    $("#emptyFilterElements").on("click", function() {
        $("#filterElements").val('');
        $("#table_manage_cmd_specific tr").show();
    });
    ...
    $("#modalSave").on("click", function() {
        ...
        for (var groupKey in tableElements){
            ...
            for (var commandKey in groupElement['command']){
                var commandElement = groupElement['command'][commandKey];
                commandElement['page'] = groupElement['page'];
                commandElement['name'] = commandElement['name'] + ' ' + groupElement['name'] + ' ' + groupKey;
